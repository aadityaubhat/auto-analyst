...
import logging
import duckdb
from csv_utils import clean_df
from sql_operator import execute_query
...
def analyze(question:str, description, data_df:pd.DataFrame) -> str:
    ...
    if agg_plot == "aggregate":
        ...
        sql_prompt = render_sql_prompt( table_name='data_table',
        ...
        transformation_description = get_chat_reply(system_transform_prompt, transformation_prompt)
        ...
        transformed_df = execute_query(prompt=sql_transform_prompt, query=sql_transform_query, data_table=data_df)
