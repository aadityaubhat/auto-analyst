...
system_plotly_prompt = "You are a helpful assistant that only writes Python code using plotly library."
...
analytical_examples = [
    ...
    {"question": "Who are the top 10 customers by number of transactions?", "is_analytical": "yes"},
...
data_examples = [
    {"description": "A CSV file with data about sales", "question" :"How many sales were made in August?", "can_be_answered": "yes"},
    {"description": "Customer complaints csv file", "question" :"What are the most common customer complaints about?", "can_be_answered": "yes"},
    {"description": "Customer visits data", "question" :"What are the top 10 selling products?", "can_be_answered": "No"},
    {"description": "Customer demographics", "question" :"Plot the histogram of customer age", "can_be_answered": "Yes"},
    {"description": "Marketting emails data", "question" :"Who is our oldest customer?", "can_be_answered": "no"},
    {"description": "Marketting emails data", "question" :"What are the best times to send the marketting emails?", "can_be_answered": "yes"},
]
...
def render_data_prompt(description, question):
    return data_template.render(examples=data_examples, description=description, question=question)
...
agg_plot_examples = [
    ...
    {"question": "Who are the top 10 customers by number of transactions?", "type": "aggregate"},
...
def render_sql_prompt(table_name, description, schema, sample_data, **kwargs):
    if kwargs.get('question'):
        ...
        return sql_template.render(table_name=table_name, description=description, schema=schema, sample_data=sample_data, question=question)
    
    if kwargs.get('transformation'):
        transformation = kwargs['transformation']
        return sql_template.render(table_name=table_name, description=description, schema=schema, sample_data=sample_data, transformation=transformation)
...
def render_plotly_prompt(table_name, sample_data, question):
    return plotly_template.render(table_name=table_name, sample_data=sample_data, question=question)
...
def render_transform_prompt(table_name, description, schema, question):
    return transform_template.render(table_name=table_name, description=description, schema=schema, question=question)
